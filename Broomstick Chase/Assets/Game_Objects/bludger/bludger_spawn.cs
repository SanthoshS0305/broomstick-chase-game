...
public class bludger_spawn : MonoBehaviour
{
    ...
    private void Spawn(){
            ...
            float y_random_pos=(float) (Math.Round((double) (UnityEngine.Random.Range(lower_bound/step, upper_bound/step)*step)));
            Vector3 spawn_pos=new Vector3(transform.position.x, y_random_pos, transform.position.z);
            Instantiate(bludger, spawn_pos, transform.rotation);
    }
}
